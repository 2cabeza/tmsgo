...
export class FormComponent implements OnInit {
  ...
  constructor(public modalController: ModalController,
    ...
    this.form = new FormGroup({
        ...
        estimated_closing_date: new FormControl(),
  ...
  onSelect(event: any, field: any){
    console.log('selects', event, field)
    let value:any;
    if(event){
      value = event && _.isArray(event) ? event : [event];
    }else{
      value = event
    }
...
/*   // search modal
  ...
  async getServiceId(id: any) {
    ...
    this.service.getServicesId(id).subscribe((response) => {
      ...
      // }
      if(this.service_item.origin){
        this.origins = [this.service_item.origin];
      }

      if(this.service_item.origin){
        this.destinations = [this.service_item.destination];
      }
